...
| 端口 | `8101` |
| 类型 | 只读查询 |
| 依赖 | 上游食堂菜品系统 API / 菜单数据库、Redis（缓存） |
...
查询校园食堂的**真实**菜单数据：某天、某餐次、某食堂/窗口供应哪些菜，含价格、辣度、主要食材、过敏原标签、余量状态与营业时间。

**不做**：营养估算（→ `nutrition-analyzer`）、排队预测（→ `crowd-forecast`）、组合推荐（→ `dish-recommender`）。
...
```
POST http://canteen-menu-query:8101/v1/{action}
GET  http://canteen-menu-query:8101/health
GET  http://canteen-menu-query:8101/openapi.json
GET  http://canteen-menu-query:8101/metrics
```

| action | 说明 |
| --- | --- |
| `query_dishes` | 按条件查菜品列表 |
| `get_dish_detail` | 单菜品详情（配料、过敏原、可能交叉污染） |
| `get_canteen_info` | 食堂/窗口信息与营业时间 |
| `check_availability` | 指定菜品余量 |

完整字段定义见 [docs/05-技能清单与接口契约.md](../../docs/05-技能清单与接口契约.md) 的 S1 章节；机器可读契约见 `openapi.yaml`（实现阶段产出）。
...
| 参数 | 类型 | 必填 | 校验规则 |
| --- | --- | --- | --- |
| `campus` / `canteen` | string | ❌ | 枚举白名单；非法值返回 `40001` |
| `date` | string(date) | ❌ | 缺省=今天；范围 [今天, +7 天]，越界返回 `40001` |
| `meal_period` | enum | ❌ | `breakfast`/`lunch`/`dinner`/`night_snack`；缺省按当前时间推断 |
| `keywords` | string[] | ❌ | 最多 5 个，单条 ≤ 20 字 |
| `filters.price_max` | number | ❌ | 0 < x ≤ 200 |
| `filters.spice_level_max` | int | ❌ | 0–3 |
...
```json
{
  ...
  "data": {
    ...
    "dishes": [
      {
        "dish_id": "d_10087", "name": "香煎鸡胸饭", "window": "3F-轻食窗口",
        "price": 12.0, "spice_level": 0, "tags": ["高蛋白", "低脂"],
        "allergens": [], "may_contain": ["大豆"],
        "availability": "in_stock", "updated_at": "2026-09-08T11:32:00+08:00"
      }
    ],
    "total": 3, "page": 1
  },
...
```json
{
  "request_id": "req_...",
  "code": 40404,
  "message": "当前条件下没有匹配的菜品。建议：放宽预算至 12 元以上，或去掉'高蛋白'标签；也可查看第一食堂其他窗口。",
...
| L3 | ChromaDB 向量召回 | 快照缺失 | `vector_recall` | "以下为历史/文档信息，可能不准确" |
...
本技能为纯读操作，天然幂等，可安全重试。不涉及任何写操作。

---

## 6. 被主智能体自动调度

本技能**不写死任何路由规则**。它把自己注册到 Tool Registry，由主智能体在 ReAct 循环中自主决定是否调用。
...
description: >
  ...
  含价格、辣度、主要食材、过敏原标签、余量状态与营业时间。
  当用户问"今天/明天/中午/晚上吃什么、有什么菜、多少钱、还有没有、几点开门"时使用。
  返回的数据带 data_version 与 updated_at，回答时应告知用户数据时间；
  若 meta.degraded 为 true，必须提示"以现场为准"。
  不要用于：营养/热量计算（用 nutrition_analyzer）、排队人流（用 crowd_forecast）、
  组合搭配推荐（用 dish_recommender）。
...
examples:
  - q: "三楼还有没有红烧肉"
    params: { canteen: "第一食堂", floor: 3, keywords: ["红烧肉"], filters: { available_only: true } }
  - q: "明天中午 15 块以内有啥"
...
用户："明天中午一食堂有没有不辣的鸡胸肉，15 块以内？我对花生过敏。"
  → Planner：需要真实菜单 → 选 canteen_menu_query
  → 参数生成：date=+1, meal_period=lunch, canteen=第一食堂,
