...
**不做**：输出任何个人消费记录或明细（合规红线，只输出聚合值）；不做精确承诺。
...
```json
{
  ...
  "meta": {
    ...
    "degraded": false, "confidence": 0.78,
    "warnings": []
  }
}
```
...
**隐私硬约束**：技能层对输出做聚合校验——任何包含个人标识、单笔消费、身份信息的字段在出口前被剥离；请求参数不接受个人级查询。
...
description: >
  ...
  当用户问"哪个窗口人少""现在去要不要排队""什么时候去不用等"时使用。
...
examples:
  - q: "现在一食堂人多吗"
    params: { canteen: "第一食堂", horizon_minutes: 30 }
```
